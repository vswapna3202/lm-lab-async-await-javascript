...
const getPromiseResponse = async() => {
	try {
		const awaitResponse = await promise;
		console.log(`Yay! Promise resolved with response: ${awaitResponse}`);
	}catch(awaitReason){
		console.log(`Boo. Promise rejected with response: ${awaitReason}`);
	}		 
};
getPromiseResponse();